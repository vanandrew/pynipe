...
import time
import logging
from typing import Dict, Any, Optional, Set, Union, TYPE_CHECKING, List, cast, TypeVar, Generic, Callable
...
class Task:
    ...
    def __init__(
        ...
        self.python_function: Optional[Callable] = None
    ...
    def _run_interface(self, resolved_inputs: Dict[str, Any]) -> None:
        ...
        if hasattr(self.interface, 'inputs'):
            interface_inputs = getattr(self.interface, 'inputs')
        ...
        if hasattr(self.interface, 'output_spec'):
            output_spec_method = getattr(self.interface, 'output_spec')
            if callable(output_spec_method):
                output_spec = output_spec_method()
                if output_spec is not None and hasattr(output_spec, 'get'):
                    get_method = getattr(output_spec, 'get')
