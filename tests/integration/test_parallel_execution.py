    ...
    def test_parallel_execution(self, mock_interface_class):
        ...
        def create_task(task_id, delay):
            with TaskContext(f"Task {task_id}") as ctx:
                ...
                ctx.task.interface = create_mock_interface(task_id, delay)
                ctx.task.inputs = {}
    ...
    def test_dependency_execution_order(self, mock_interface_class):
        ...
        def process_with_dependencies():
            ...
            with TaskContext("Task 1") as ctx1:
                ctx1.task.interface = create_mock_interface(1)
                ctx1.task.inputs = {}
                outputs1 = ctx1.task.run()
                output_file1 = outputs1["out_file"]
            
            # Second task depends on first
            with TaskContext("Task 2") as ctx2:
                ctx2.task.interface = create_mock_interface(2)
                ctx2.task.inputs = {"in_file": output_file1}
                outputs2 = ctx2.task.run()
                output_file2 = outputs2["out_file"]
            
            # Third task depends on second
            with TaskContext("Task 3") as ctx3:
                ctx3.task.interface = create_mock_interface(3)
                ctx3.task.inputs = {"in_file": output_file2}
                outputs3 = ctx3.task.run()
        ...
        # Run the workflow
        results = workflow.run(executor=LocalExecutor(max_workers=4))
